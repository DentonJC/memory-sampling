...
class NUDER(SupervisedTemplate):
    """
    ...
    def __init__(
        ...
        *,
        ...
        ti=False,
        remove_current=None,
        trial_custom_probs: Optional[np.ndarray] = None,  # New arg
        **kwargs
    ):
        """
        ...
        self.alpha = alpha
        self.beta = beta
        self.trial_custom_probs = trial_custom_probs
    ...
    def _sample_from_buffer_with_probs(self):
        ...
        if len(buffer) == 0 or self.batch_size_mem == 0:
            return None
        if self.trial_custom_probs is not None and all(p == self.trial_custom_probs[0] for p in self.trial_custom_probs[:len(buffer)]):
            # Use same sampling as when None is passed, but keep using the custom code path
            indices = np.random.choice(len(buffer), size=self.batch_size_mem, 
                                      replace=len(buffer) < self.batch_size_mem)
        else:
            # Normalize and clip probs
            probs = self.trial_custom_probs[:len(buffer)] if self.trial_custom_probs is not None else None
            if probs is not None:
                probs = np.asarray(probs)
                probs = probs[:len(buffer)]
                probs = probs / probs.sum()
                indices = np.random.choice(len(buffer), size=self.batch_size_mem, replace=len(buffer) < self.batch_size_mem, p=probs)
            else:
                indices = np.random.choice(len(buffer), size=self.batch_size_mem, replace=len(buffer) < self.batch_size_mem)
            
        samples_x, samples_y, samples_t, samples_l = [], [], [], []
        ...
        for idx in indices:
            ...
            samples_x.append(x.unsqueeze(0))
            samples_y.append(y)
            samples_l.append(l.unsqueeze(0))
            if t is not None:
                samples_t.append(t)

        x_tensor = torch.cat(samples_x).to(self.device)
        y_tensor = torch.tensor(samples_y).to(self.device)
        l_tensor = torch.cat(samples_l).to(self.device)


        if self.ti:
            t_tensor = torch.tensor(samples_t).to(self.device)
            return x_tensor, y_tensor, t_tensor, l_tensor
        else:
            return x_tensor, y_tensor, None, l_tensor
    ...
    def _before_forward(self, **kwargs):
        ...
        if self.trial_custom_probs is not None:
            result = self._sample_from_buffer_with_probs()
            if result is None:
                return
            batch_x, batch_y, batch_tid, batch_logits = result
        elif self.replay_loader is not None:
            samples = next(self.replay_loader)
            if self.ti:
                batch_x, batch_y, batch_tid, batch_logits = [x.to(self.device) for x in samples]
            else:
                batch_x, batch_y, batch_logits = [x.to(self.device) for x in samples[:2]]
                batch_tid = None
        else:
            return


        batch_x, batch_y, batch_logits = (
